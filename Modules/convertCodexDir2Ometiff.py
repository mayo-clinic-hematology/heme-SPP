...
def write_ometiff(targetDir, targetPrefix, outpath, pxSize):
	...
	stepwiseTable.loc[stepwiseTable["Marker"].str.startswith("DAPI", na = False), 'Keep'] = 'Yes'
	...
	"""
	## Create marker.csv information file for compatibility with MCMICRO
	for index, row in stepwiseTable.iterrows():
		
		stepwiseTable.at[index, 'channel_number'] = str(int(index + 1))
		stepwiseTable.at[index, 'cycle_number'] = row["Cycle"][3:].lstrip("0")
		stepwiseTable.at[index, 'marker_name'] = row['Marker']
		stepwiseTable.at[index, 'Filter'] = channelDesign[row['Channel']]

	MCMicro_table = stepwiseTable.loc[stepwiseTable['Keep'] == "Yes"]
	MCMicro_table = MCMicro_table[["channel_number","cycle_number", "marker_name", "Filter"]]
	MCMicro_table.to_csv(os.path.join(args.outDir, 'markers.csv'), index=False)

	## Write ome-tiff
	tifffile.imwrite(outpath, data=imarr.values, description=xml, contiguous=True )
	## Drop the duplicates column created previously
	stepwiseTable.drop(["duplicates", "channel_number","cycle_number", "marker_name", "Filter"], axis=1, inplace=True)
