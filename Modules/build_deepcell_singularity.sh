...
apptainer build initialdeepcell.sif docker://vanvalenlab/deepcell-applications

## modify the container and re-build it
apptainer build --sandbox mysandbox initialdeepcell.sif 
cp mysandbox/usr/src/app/run_app.py ../
chmod ug+x ../run_app.py
echo "export PYTHONPATH=/usr/src/app" >> mysandbox/environment
apptainer build ../deepcell.sif mysandbox/
